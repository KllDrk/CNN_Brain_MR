    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                flash('Başarıyla giriş yaptı!', category='success')
                login_user(user, remember=True)
                return redirect(url_for('views.home'))
            else:
                flash('Yanlış şifre, tekrar deneyin.', category='error')
        else:
            flash('E-posta mevcut değil.', category='error')
...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('E-posta 3 karakterden uzun olmalıdır.', category='error')
        elif len(first_name) < 2:
            flash('İsminiz 2 karakterden uzun olmalıdır.', category='error')
        elif password1 != password2:
            flash('Parolalar eşleşmiyor.', category='error')
